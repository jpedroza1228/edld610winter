...
# library(ggalt)
pacman::p_load(rio, tidyverse, here, nhanesA, xaringan, ggridges, viridis, ggalt)
...
complete <- complete %>%
  ...
  mutate(data_cycle = case_when(sddsrvyr == 7 ~ "2011_2012",
         ...
         gender = case_when(riagendr == 1 ~ "male",
                            ...
                            TRUE ~ NA_character_),
         age = ridageyr,
         race = case_when(ridreth3 == 1 ~ "mexican",
                       ...
                       ridreth3 == 7 ~ "multi",
                       TRUE ~ NA_character_),
         birth_country = case_when(dmdborn4 == 1 ~ "us_born",
                                dmdborn4 == 2 ~ "other",
                                TRUE ~ NA_character_),
         citizen = case_when(dmdcitzn == 1 ~ "citizen",
                          dmdcitzn == 2 ~ "not_citizen",
                          TRUE ~ NA_character_),
         length_us = case_when(dmdyrsus == 1 ~ "less_year",
                            dmdyrsus == 2 ~ "1_5yr",
                            dmdyrsus == 3 ~ "5_10yr",
                            dmdyrsus == 4 ~ "10_15yr",
                            dmdyrsus == 5 ~ "15_20yr",
         ...
         length_us = case_when(dmdyrsus == 1 ~ "less_year",
                            ...
                            dmdyrsus == 9 ~ "50_plus_yr",
                            TRUE ~ NA_character_),
         ed = case_when(dmdeduc2 == 1 ~ "less_9",
                     dmdeduc2 == 2 ~ "9_11",
                     dmdeduc2 == 3 ~ "highschool",
                     dmdeduc2 == 4 ~ "aa",
                     dmdeduc2 == 5 ~ "college_grad",
                     TRUE ~ NA_character_),
         household = case_when(dmdfmsiz == 1 ~ "1",
                            dmdfmsiz == 2 ~ "2",
                            dmdfmsiz == 3 ~ "3",
                            dmdfmsiz == 4 ~ "4",
                            dmdfmsiz == 5 ~ "5",
                            ...
                            dmdfmsiz == 7 ~ "7_plus",
                            TRUE ~ NA_character_),
         income = case_when(indhhin2 == 1 ~ "0_5k",
                         indhhin2 == 2 ~ "5_10k",
                         indhhin2 == 3 ~ "10_15k",
                         indhhin2 == 4 ~ "15_20k",
                         indhhin2 == 5 ~ "20_25k",
         ...
         mod_rec_pa = pad675,
         sed = pad680,
         days_active_60 = case_when(paq706 == 0 ~ "0_day",
                                 paq706 == 1 ~ "1_day",
                                 paq706 == 2 ~ "2_day",
                                 paq706 == 3 ~ "3_day",
                                 paq706 == 4 ~ "4_day",
                                 ...
                                 paq706 == 7 ~ "7_day",
                                 TRUE ~ NA_character_),
         hours_tv = case_when(paq710 == 0 ~ "less_hour",
                             paq710 == 1 ~ "1_hour",
                             paq710 == 2 ~ "2_hours",
                             paq710 == 3 ~ "3_hours",
                             paq710 == 4 ~ "4_hours",
                             ...
                             paq710 == 8 ~ "no_tv",
                           TRUE ~ NA_character_),
         hours_comp = case_when(paq715 == 0 ~ "less_hour",
                             paq715 == 1 ~ "1_hour",
                             paq715 == 2 ~ "2_hours",
                             paq715 == 3 ~ "3_hours",
                             paq715 == 4 ~ "4_hours",
...
# Yufei: just looks a little bit concise to put them together!
...
data <- complete %>%
  select(-2:-19) %>% 
  mutate(age_cut = cut(age, breaks = c(0, 18, 25, 55, Inf),
                    labels = c("minor", "young adult", "adult",
                               "older adult")))
# Yufei: again, for me it is a little lit easier to understand when putting together
...
ggplot(data, aes(age, fill = age_cut)) +
  geom_histogram(color = "white", # Yufei: The white line added to the col is so helpful for a clearer view of the plot; I will do that to my plots in the future.
                 alpha = .7,
                 bins = 50) + 
...
# Yufei: I really like the color set you chose for the plots, but i just feel the yellow color is not that clear when it overlays on the grey background
```
...
# Yufei: there are some overlap on the xlab and I think we can fix it by adding the width of the plot
...
ggplot(data, aes(mod_work, vig_work)) +
  ...
  geom_encircle(aes(mod_work, vig_work), # Yufei: this function is so cool! I like these plots very much and I am learning from your codes to use it!
                data = less_yr,
                color = "red",
                size = 2,
                expand = .04) +
...
# Yufei: the color set you chose is very pretty, but I think it would be best for continuous data; here the race is discrete, and i was wondering if discrete color set makes better sense in presenting their relationships?
```
...
# I am a little bit confused by the meaning of sed. Probably it would be good to change the name? I know this is just an exploratory plot anyway..
...
ggplot(data, aes(race)) +
  ...
  facet_wrap(~data_cycle) +
  coord_flip() +
  scale_fill_viridis_d() # Yufei: the facet here makes the plot really concise and straightforward!
